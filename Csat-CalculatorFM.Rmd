...
df <- read.csv(paste(results_dir, "20210618_sfpqQG\\20x\\Method#2 (calculate total condensed volume)_CA30.csv", sep = ""))
```
...
ggplot(data = df, mapping = aes(x = Protein_Conc_uM, y = Cond_Vol_in_sample_nL)) +
   geom_point() +
   labs(title = "SFPQ(QG)-mEGFP 18/6/2021")
```
...
# Truncate results table to include only data > Csat...
rows_to_keep <- df$Protein_Conc_uM > Csat_approx
df_trunc <- df[rows_to_keep,]
...
ggplot(data = df, mapping = aes(x = Protein_Conc_uM, y = Cond_Vol_in_sample_nL)) +
   ...
   ylim(0, max(df$Cond_Vol_in_sample_nL)) +
   geom_segment(aes(x = Csat, 
                    y = 0, 
                    xend = Csat,
                    yend = max(df$Cond_Vol_in_sample_nL)/2),
                linetype = "dashed") +
   geom_label(
      label = paste("C(sat) = ", Csat, " uM", sep = ""),
      x = Csat,
      y = max(df$Cond_Vol_in_sample_nL)/2,
      size = 6,
      colour = "darkred") +
