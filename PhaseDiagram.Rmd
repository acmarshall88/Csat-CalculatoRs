---
title: "Calculate Csat and condensed phase [protein]"
subtitle: "Data = 20230421_ECsfpqWTcleaved_pheno/5hr/10x/GFP"
...
2. Plots the *total condensed protein volume* over the *total protein concentration*
...
5. Calculates [protein] within condensed phase ** (*only accurate if condensed phase volume has been measured accurately*)
...
```{r get data}
### USER input: ###
results_dir <- "\\\\uniwa.uwa.edu.au\\userhome\\staff7\\00101127\\My Documents\\LLPS results\\20230421_ECsfpqWTcleaved_pheno\\5hr\\10x\\GFP\\"
df <- read.csv(paste(results_dir, "Method#2 (calculate total condensed volume).csv", sep = ""))
```
...
df_collapse <- summaryBy(Cond_Vol_in_sample_nL ~ Protein_Conc_uM, 
                         data = df, 
                         FUN = c(length, mean, sd))
# Rename columns:
names(df_collapse)[names(df_collapse)=="Cond_Vol_in_sample_nL.length"] <- "N"
names(df_collapse)[names(df_collapse)=="Cond_Vol_in_sample_nL.mean"] <- "Cond_Vol_in_sample_nL"
names(df_collapse)[names(df_collapse)=="Cond_Vol_in_sample_nL.sd"] <- "sd"
###############################################
...
plt_Csat <- 
ggplot(data = df_collapse, mapping = aes(x = Protein_Conc_uM, y = Cond_Vol_in_sample_nL)) +
   # theme(text = element_text(size=20)) +
   geom_point(size = 1.5) +
   geom_errorbar(aes(ymin=Cond_Vol_in_sample_nL-sd, ymax=Cond_Vol_in_sample_nL+sd), 
                 width=max(df$Protein_Conc_uM*0.04)) +
   # labs(title = "H6-mEGFP-SFPQ condensation",
   #      subtitle = "150 mM KCl, 20 mM HEPES pH 7.4\n(room temperature)") +
   xlab("total [protein] (\u03bcM)") +
   ylab("Condensed Vol (nL)") +
   # theme(
   ...
   xlim(0, max(df_trunc$Protein_Conc_uM)*1.05) +
   geom_segment(aes(x = Csat, 
                    y = 0, 
                    xend = Csat,
                    yend = max(df_trunc$Cond_Vol_in_sample_nL)/2.2),
                linetype = "dashed",
                color = "darkred") +
   annotate(
      "text",
...
```{r}
### USER INPUT: ###
   # Total volume (in uL) of each sample here:
   sample_vol_uL <- 14
...
v_tot <- sample_vol_uL*10^-6                # <-Sample Volume (L)
